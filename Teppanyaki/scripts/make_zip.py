...
def  main():
  ...
  installer = "\installer\Teppanyaki Installer.exe"
   
  if demo:
    installer = "\installer\Teppanyaki Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\Teppanyaki manual.pdf" 
  ]
  ...
  ZIPNAME = "Teppanyaki-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "Teppanyaki-v" + FULLVERSIONSTR + "-win-demo.zip"
